...
def generate_audio(
    prompt_audio, prompt_text, target_text, temperature, top_p, top_k, max_new_tokens
):
...
with gr.Blocks(title="Fish Speech TTS WebUI", theme=gr.themes.Soft()) as app:
    ...
    gr.Markdown(
        "Zero-shot/Few-shot voice cloning and generation based on Dual-AR model (supports automatic long text slicing)"
    )
    ...
    with gr.Accordion("⚙️ Model Settings", open=True):
        ...
        with gr.Row():
            ...
            load_status = gr.Textbox(
                label="Load Status",
                interactive=False,
                value="Model not loaded, please click the button on the left.",
                scale=3,
            )

        load_btn.click(
            fn=load_models,
            inputs=[model_path_input, vqgan_ckpt_input],
            outputs=[load_status],
        )
        ...
        with gr.Column(scale=1):
            ...
            with gr.Accordion("🔧 Advanced Settings", open=False):
                ...
                top_k_slider = gr.Slider(
                    minimum=1, maximum=100, value=30, step=1, label="Top-k"
                )
